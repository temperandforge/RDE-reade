    ...
    if (isset($hero_breadcrumbs) && count($hero_breadcrumbs)) {
        ...
        <p class="blog-hero-breadcrumb">
            ...
            if (!empty($_GET['from']) && is_numeric($_GET['from'])) {
                ...
                $from = (int) $_GET['from'];
                if ($cat = get_term($from, 'category')) {
                    $hb_url = get_term_link($from, 'category');
                    $hb_text = 'Back To ' . $cat->name;

                    ?>
                        <a href="<?php echo $hb_url; ?>"><svg class="first-arrow" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
