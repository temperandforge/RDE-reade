			...
			<div class="tf-dropdown-list">
				...
				if (!empty($opts['values'])) {
					...
					foreach ($opts['values'] AS $key => $value) {
						$output .= '<li data-key="' . $key . '">' . $value . '</li>';
					}
				}
...
function tf_dropdown_js($return) {
	...
	$js = '<script>let clickOutside=function(e){e.preventDefault();let t=document.getElementsByClassName("tf-dropdown");for(i=0;i<t.length;i++)t[i].contains(e.target)||t[i].classList.remove("tf-dropdown-open")};function toggleDropdown(e){let t=document.getElementById(e);t.classList.contains("tf-dropdown-open")?(t.classList.remove("tf-dropdown-open"),document.removeEventListener("click",clickOutside)):(t.classList.add("tf-dropdown-open"),document.addEventListener("click",clickOutside))}document.addEventListener("DOMContentLoaded",function(){let e=document.getElementsByClassName("tf-dropdown");if(e.length)for(let t=0;t<e.length;t++){e[t].addEventListener("click",function(){toggleDropdown(e[t].getAttribute("id"))});let n=e[t].querySelectorAll("li");if(n.length)for(let o=0;o<n.length;o++)n[o].addEventListener("click",function(){e[t].querySelector("dt p").innerText=n[o].innerText})}});</script>';
